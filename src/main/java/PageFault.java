...
public class PageFault {
    ...
    public static void replacePage(Vector mem, int virtPageNum, int replacePageNum, ControlPanel controlPanel) {
        ...
        int count = 0;
        int oldestPage;
        List<Page> zeroR = new ArrayList<>();
        List<Page> zeroM = new ArrayList<>();
        List<Page> zeroMR = new ArrayList<>();
        List<Page> MR = new ArrayList<>();
        for (int i = 0; i < virtPageNum; i++) {
            ...
            if(!zeroMR.isEmpty()){
               oldestPage = random.nextInt(zeroMR.size());
            } else if(!zeroM.isEmpty()){
                oldestPage = random.nextInt(zeroM.size());
            }else {
                oldestPage = random.nextInt(zeroR.size());
            }
